...
INSERT INTO public.campaign_vendor_requests (campaign_id, vendor_id, playlist_ids, status, response_notes, requested_at)
SELECT 
  c.id as campaign_id,
  v.id as vendor_id,
  CASE 
    WHEN c.name LIKE '%Hip Hop%' THEN '["' || p1.id || '","' || p2.id || '"]'::jsonb
    ELSE '["' || p1.id || '"]'::jsonb
  END as playlist_ids,
  CASE 
    WHEN c.status = 'active' THEN 'approved'
    WHEN c.status = 'draft' THEN 'pending'
    ELSE 'rejected'
  END as status,
  CASE 
    WHEN c.status = 'rejected' THEN 'Does not fit our playlist audience'
    WHEN c.status = 'active' THEN 'Great track, perfect for our playlists!'
    ELSE NULL
  END as response_notes,
  c.created_at as requested_at
FROM campaigns c
CROSS JOIN vendors v
LEFT JOIN playlists p1 ON p1.vendor_id = v.id
LEFT JOIN playlists p2 ON p2.vendor_id = v.id AND p2.id != p1.id
