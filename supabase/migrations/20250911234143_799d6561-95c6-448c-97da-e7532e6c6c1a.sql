-- Drop and recreate vendor campaign visibility policies to fix Club Restricted campaign visibility
-- and support both string and object formats in selected_playlists

-- Drop existing vendor campaign policies first
DROP POLICY IF EXISTS "Vendors can view assigned campaigns" ON public.campaigns;

-- Create comprehensive vendor campaign visibility policies
-- 1) Allow vendors to view campaigns that have requests addressed to them
CREATE POLICY "Vendors can view campaigns with requests"
ON public.campaigns
FOR SELECT
USING (
...
CREATE POLICY "Vendors can view campaigns via playlist allocation"
ON public.campaigns
FOR SELECT
USING (
  public.is_vendor() AND EXISTS (
...
CREATE POLICY "Vendors can view campaigns via vendor_allocations"
ON public.campaigns
FOR SELECT
USING (
  public.is_vendor() AND EXISTS (
