...
import { CalendarDays, Download, DollarSign, TrendingUp } from "lucide-react";
...
export default function AdminPaymentHistoryPage() {
  ...
  const filteredPayments = payments.filter(payment =>
    (payment.vendor as any)?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="container mx-auto px-6 py-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Comprehensive payment tracking and analysis</p>
          </div>
        ...
        <div className="flex items-center space-x-4">
          <Input
            placeholder="Search by vendor, campaign, or reference..."
            value={searchTerm}
                ...
                {filteredPayments.map((payment) => (
                  <TableRow key={payment.id}>
                    ...
                    <TableCell className="font-medium">
                      {(payment.vendor as any)?.name || "Unknown Vendor"}
                    </TableCell>
                    <TableCell>{(payment.campaign as any)?.name || "Unknown Campaign"}</TableCell>
