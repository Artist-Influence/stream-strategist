    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.goalCompletionRate.toFixed(1)}%</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Avg Cost/Stream</CardTitle>
            <DollarSign className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">${data.averageCostPerStream.toFixed(3)}</div>
            <p className="text-xs text-muted-foreground flex items-center">
              <span className={`flex items-center ${
                data.performanceBenchmarks.ourAvgCostPerStream < data.performanceBenchmarks.industryAvgCostPerStream 
                ...
                vs industry avg ${data.performanceBenchmarks.industryAvgCostPerStream}
              </span>
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatPercentage(data.monthOverMonthGrowth.campaigns)}</div>
            <p className="text-xs text-muted-foreground">Campaign growth MoM</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground flex items-center">
              <span className={`flex items-center ${
                data.averageROI > data.performanceBenchmarks.industryAvgROI 
                  ? 'text-green-600' : 'text-red-600'
              }`}>
                {data.averageROI > data.performanceBenchmarks.industryAvgROI 
                  ? <ArrowUpRight className="h-3 w-3 mr-1" />
                ...
                vs industry {data.performanceBenchmarks.industryAvgROI}%
              </span>
            </p>
          </CardContent>
        </Card>
